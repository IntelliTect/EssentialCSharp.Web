...
public sealed class BookSearchTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string assessment;
        if (hasHeadingCoverage && (hasSemanticCoverage || !semanticAvailable))
        {
            assessment = "**Comprehensive** — dedicated section headings found";
        }
        else if (hasHeadingCoverage)
        {
            assessment = "**Mentioned** — appears in section headings";
        }
        else if (hasSemanticCoverage)
        {
            assessment = "**Mentioned** — referenced in book content but no dedicated heading";
        }
        else if (!semanticAvailable)
        {
            assessment = hasHeadingCoverage
                ? "**Mentioned** — appears in section headings (semantic search unavailable for deeper analysis)"
    ...
    [McpServerTool(Title = "Find Book Help For Diagnostic", ReadOnly = true, Destructive = false, Idempotent = true, OpenWorld = false),
     Description("Find Essential C# book sections and guidelines that help explain a C# compiler error, warning, or runtime exception. Accepts a CS diagnostic code (e.g., 'CS8600') or a plain description (e.g., 'null reference exception', 'cannot implicitly convert'). Returns relevant book sections and guidelines.")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Map well-known CS codes to relevant search topics
        ...
        // Heading search on mapped topic
        var headingMatches = _SiteMappingService.SiteMappings
        ...
        if (_SearchService is not null)
        {
            ...
            if (vectorResults.Count > 0)
            {
                ...
                foreach (var r in vectorResults)
                {
                    ...
                    {
                        sb.AppendLine(CultureInfo.InvariantCulture, $"**{r.Record.Heading}** (Ch. {r.Record.ChapterNumber})");
                    }
                    sb.AppendLine(r.Record.ChunkText);
                    sb.AppendLine();
                }
            }
        }

        if (headingMatches.Count == 0 && _SearchService is null)
        {
            sb.AppendLine(CultureInfo.InvariantCulture, $"No sections found for '{searchTerm}'. Semantic search is unavailable in this environment.");
        }
